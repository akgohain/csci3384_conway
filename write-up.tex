...
\subsection{PERN in NP:}


We formally define the Pattern Reachability Problem (PERN) for Conway's Game of Life (GoL) and prove its NP membership.
...
\subsubsection{NP Certificate Based Proof: }
A language is in NP if there exists a polynomial-time verifier for its "yes" instances. We construct such a verifier:
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item To update all $n$ cells in configuration takes $n*O(1)$ or $O(n)$ time. This our "per-step" time.
...
\subsection{What about our Still lifes, Oscillators, and Spaceships?}
